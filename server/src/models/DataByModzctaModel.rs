...
pub fn read(conn: &PgConnection) -> Vec<(DataByModzctaT, ZctaToModzctaT)> {
    DataByModzcta::table
        .left_join(ZctaToModzcta::table.on(ZctaToModzcta::MODZCTA.eq(DataByModzcta::MODIFIED_ZCTA)))
        // .select((DataByModzcta::MODIFIED_ZCTA, ZctaToModzcta::MODZCTA))
        .load::<(DataByModzctaT, ZctaToModzctaT)>(conn)
