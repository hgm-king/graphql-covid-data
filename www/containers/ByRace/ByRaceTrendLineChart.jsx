...
export default function ByRaceLineTrendLineChart( props )  {
  ...
  const setSelectedFieldHandler = ({target}) => { setSelectedField(target.value) }
  const setSelectedCalculationHandler = ({target}) => { setSelectedCalculation(target.value) }
  ...
  return (
    <div>
      ...
      <select selected={selectedCalculation} onChange={setSelectedCalculationHandler}>
