...
export default function ByRace( props )  {
  ...
  const pieKeys = Object.keys(dataForDay[0]).filter(key => key.match(/COUNT/)).filter(key => !key.match(/CASE/))
  const raceIndexes = [...new Set(data.ByRace.map(getIndex).sort())].filter(d => d)
